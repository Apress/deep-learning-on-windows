    ...
    for game_index in range(intial_games):
        ...
        if score >= score_requirement:
            ...
            for data in game_memory:
                output = np_utils.to_categorical(data[1], 2)
...
for each_game in range(100):
    ...
    for step_index in range(goal_steps):
        # Keep the below line uncommented if you want to see how our bot is playing the game.
        env.render()
        if len(prev_obs)==0:
            action = random.randrange(0,2)
        else:
        ...
        if done:
            ...
            if (step_index + 1) > 400:
                # if achieved more than 400 steps, consider successful
                success_count += 1
            break
