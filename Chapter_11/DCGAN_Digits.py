...
def generate_and_save_images(model, epoch, test_input, display = False):
    ...
    plt.savefig('output/image_at_epoch_{:04d}.png'.format(epoch), facecolor=fig.get_facecolor())
    plt.close()
    disp_image = cv2.imread('output/image_at_epoch_{:04d}.png'.format(epoch))
    disp_image = cv2.bitwise_not(disp_image)
    cv2.imwrite('output/image_at_epoch_{:04d}.png'.format(epoch), disp_image)
    if (display):
